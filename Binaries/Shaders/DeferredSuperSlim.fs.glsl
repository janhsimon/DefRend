...
uniform usampler2D inGBufferMRT0;
uniform usampler2D inGBufferMRT1;
...
vec3 unpackNormal(vec2 uv)
{
	...
	uint nr = texture(inGBufferMRT1, uv).r;
	uint ng = texture(inGBufferMRT1, uv).g;
...
{
	uint depth = texture(inGBufferMRT1, uv).z << 8;
	depth = depth | texture(inGBufferMRT1, uv).a;
	return (float(depth) / 65535.0) * cameraFarClip;
}
...
void main()
{
	...
	vec3 diffuseTextureColor = vec3(texture(inGBufferMRT0, uv).rgb) / 255.0;
	...
	// point or spot light
	{
		vec3 worldPosition = reconstructFromDepth(unpackDepth(uv));
		...
		if (diffuseFactor > 0.0)
		{
			...
			float materialSpecularReflectivity = float(texture(inGBufferMRT1, uv).a) / 255.0;
